---
id: "@specs/aws/opensearchserverless/docs/gsg-serverless-cli"
version: 1.0.0
target_lang: meta
owned-by: aws-docs
source: "AWS Tutorial: Getting started with security (CLI)"
status: active
depends_on:
  - "@specs/aws/opensearchserverless/meta"
---

# Tutorial: Getting started with security (CLI)

> **source:** AWS Documentation
> **spec:id:** @specs/aws/opensearchserverless/docs/gsg-serverless-cli
...
This tutorial covers the steps described in the [console getting started tutorial](gsg-serverless.md) for security, but uses the AWS CLI rather than the OpenSearch Service console. 

You complete the following steps in this tutorial:

1. Create an IAM permissions policy

1. Attach the IAM policy to an IAM role
...
1. Upload data to your collection
...
1. Attach `TutorialPolicy` to the IAM role who will index and search data in the collection. In this example, the role is named `TutorialRole`:
...
1. Before you create a collection, you need to create an [encryption policy](serverless-encryption.md) that assigns an AWS owned key to the *books* collection that you create in a later step.
...
1. Use an HTTP tool such as [Postman](https://www.getpostman.com/) or curl to index data into the *books* collection. The following example creates an index called *books-index* and adds a single document.
