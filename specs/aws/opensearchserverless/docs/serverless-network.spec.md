---
id: "@specs/aws/opensearchserverless/docs/serverless-network"
version: 1.0.0
target_lang: meta
owned-by: aws-docs
source: "AWS Network access"
status: active
depends_on:
...
> **spec:id:** @specs/aws/opensearchserverless/docs/serverless-network
...
+ If a collection is accessible from public networks, it's also accessible from all OpenSearch Serverless-managed VPC endpoints and all AWS services. Even with public network access enabled, data access policies still control who can read and write data to the collection. Network access only determines which networks can reach the collection endpoint; data access policies determine which principals can perform operations on the data. For more information, see [Data access control for Amazon OpenSearch Serverless](serverless-data-access.md).
