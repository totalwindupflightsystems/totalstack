---
id: "@specs/aws/ec2/delete_verified_access_instance"
version: 1.0.0
target_lang: py
owned-by: codegen
model_pool: code-gen
status: active
depends_on:
  - "@specs/aws/ec2/plan"
  - "@specs/aws/ec2/docs/API_DeleteVerifiedAccessInstance"
---

# DeleteVerifiedAccessInstance

> **spec:trace:** specs/aws/ec2/ec2.spec.plan.md#operation-inventory
> **spec:id:** @specs/aws/ec2/delete_verified_access_instance
> **spec:implements:** @kind:operation DeleteVerifiedAccessInstance
> **AWS Protocol:** ec2
> **HTTP:** POST /
> **@ref:** specs/aws/ec2/docs/API_DeleteVerifiedAccessInstance.spec.md

Delete an Amazon Web Services Verified Access instance.

## Input Shape: DeleteVerifiedAccessInstanceRequest

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| ClientToken | str |  | A unique, case-sensitive token that you provide to ensure idempotency of your modification request. For more information |
| DryRun | bool |  | Checks whether you have the required permissions for the action, without actually making the request, and provides an er |
| VerifiedAccessInstanceId | Any  # complex shape | ✓ | The ID of the Verified Access instance. |

## Output Shape: DeleteVerifiedAccessInstanceResult

- **VerifiedAccessInstance** (Any  # complex shape): Details about the Verified Access instance.

## Implementation

```speclang
def delete_verified_access_instance(store, request: dict) -> dict:
    """Delete an Amazon Web Services Verified Access instance."""
    verified_access_instance_id = request.get("VerifiedAccessInstanceId", "").strip() if isinstance(request.get("VerifiedAccessInstanceId"), str) else request.get("VerifiedAccessInstanceId")

    if not store.verified_access_instances(verified_access_instance_id):
        raise ResourceNotFoundException(f"Resource verified_access_instance_id not found")
    store.delete_verified_access_instances(verified_access_instance_id)
    return {}
```
