---
id: "@specs/aws/lambda/delete_event_source_mapping"
version: 1.0.0
target_lang: py
owned-by: codegen
model_pool: code-gen
...
depends_on:
  ...
  - "@specs/aws/lambda/docs/API_DeleteEventSourceMapping"
---

# DeleteEventSourceMapping

> **spec:trace:** specs/aws/lambda/lambda.spec.plan.md#operation-inventory
> **spec:id:** @specs/aws/lambda/delete_event_source_mapping
> **spec:implements:** @kind:operation DeleteEventSourceMapping
> **AWS Protocol:** rest-json
> **HTTP:** DELETE /2015-03-31/event-source-mappings/{UUID}
> **@ref:** specs/aws/lambda/docs/API_DeleteEventSourceMapping.spec.md
...
```speclang
def delete_event_source_mapping(store, request: dict) -> dict:
    """Deletes an event source mapping . You can get the identifier of a mapping from the output of ListEventSourceMappings . When you delete an event source mapping, it enters a Deleting state and might not"""
    uuid = request.get("UUID", "").strip() if isinstance(request.get("UUID"), str) else request.get("UUID")

    if not store.event_source_mappings(uuid):
        raise ResourceNotFoundException(f"Resource uuid not found")
    store.delete_event_source_mappings(uuid)
    return {}
```
