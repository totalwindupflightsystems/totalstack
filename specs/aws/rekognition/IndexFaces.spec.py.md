---
id: "@specs/aws/rekognition/index_faces"
target_lang: py
version: 1.0.0
owned-by: codegen
status: active
...
|-------|------|----------|-------------|
| `CollectionId` | CollectionId | Yes | The ID of an existing collection to which you want to add the faces that are detected in the input i |
| `DetectionAttributes` | Attributes | No | An array of facial attributes you want to be returned. A DEFAULT subset of facial attributes - Bound |
| `ExternalImageId` | ExternalImageId | No | The ID you want to assign to all the faces detected in the image. |
| `Image` | Image | Yes | The input image as base64-encoded bytes or an S3 object. If you use the AWS CLI to call Amazon Rekog |
| `MaxFaces` | MaxFacesToIndex | No | The maximum number of faces to index. The value of MaxFaces must be greater than or equal to 1. Inde |
