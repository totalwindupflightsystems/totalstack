---
id: "@specs/aws/dynamodb/create-table"
version: 1.0.0
target_lang: py
owned-by: codegen
model_pool: code-gen
...
short: "CreateTable — Adds a new table to your account"
---
...
The `CreateTable` operation adds a new table to your account. In an Amazon Web Services account, table names must be unique within each Region. `CreateTable` is an asynchronous operation; upon receiving a `CreateTable` request, DynamoDB immediately returns a response with a `TableStatus` of `CREATING`. After the table is created, DynamoDB sets the `TableStatus` to `ACTIVE`. You can perform read and write operations only on an `ACTIVE` table. You can optionally define secondary indexes on the new table, as part of the `CreateTable` operation.
