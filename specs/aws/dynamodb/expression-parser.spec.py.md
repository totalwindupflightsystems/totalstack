---
id: "@specs/aws/dynamodb/expression-parser"
version: 1.1.0
target_lang: py
owned-by: codegen
model_pool: code-gen
...
tags: [aws, dynamodb, expressions, parser, condition, update, projection, filter]
short: "DynamoDB expression language parser — ConditionExpression, UpdateExpression, FilterExpression, ProjectionExpression, KeyConditionExpression"
---
...
A complete recursive-descent parser and evaluator for all five DynamoDB expression types. Handles ExpressionAttributeNames (`#name`) and ExpressionAttributeValues (`:value`) placeholder resolution. Supports all DynamoDB data types with proper boxing/unboxing.

## Expression Types

- **ConditionExpression** — PutItem, UpdateItem, DeleteItem (conditional writes)
- **FilterExpression** — Query, Scan (server-side post-filter)
- **KeyConditionExpression** — Query (partition key equality + sort key range)
- **UpdateExpression** — UpdateItem (SET, REMOVE, ADD, DELETE)
- **ProjectionExpression** — GetItem, Query, Scan (attribute selection)
