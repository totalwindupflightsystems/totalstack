...
tags: [aws, dynamodb, scan]
short: "Scan operation — sequential scan of every item in a table or index"
---

# Scan

> **spec:trace:** specs/aws/dynamodb/dynamodb.spec.plan.md#phase-1--core-crud-poc
> **source:** AWS DynamoDB API Reference — Scan

The `Scan` operation returns one or more items and item attributes by accessing every item in a table or a secondary index. To return fewer items, provide a `FilterExpression`.

If the total size of scanned items exceeds the maximum dataset size limit of 1 MB, the scan completes and results are returned. `LastEvaluatedKey` is returned so the requestor can continue the scan in a subsequent operation. Each scan response includes `ScannedCount` (items scanned before filtering) and `Count` (items remaining after filtering). If no `FilterExpression` is used, `Count` equals `ScannedCount`.

For faster performance on large tables, use parallel scans with `Segment` and `TotalSegments`. Each worker scans its assigned segment. Segment IDs are zero-based; `LastEvaluatedKey` from a parallel scan must be used with the same segment ID. `ConsistentRead=true` is not supported on global secondary indexes.

**Required:** `TableName`
**Input shape:** `ScanInput`
**Output shape:** `ScanOutput` (Items, Count, ScannedCount, LastEvaluatedKey, ConsumedCapacity)
