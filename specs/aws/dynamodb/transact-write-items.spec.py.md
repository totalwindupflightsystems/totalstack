---
id: "@specs/aws/dynamodb/transact-write-items"
version: 1.0.0
target_lang: py
owned-by: codegen
model_pool: code-gen
...
tags: [aws, dynamodb, transact-write-items]
short: "TransactWriteItems operation — atomic write set with idempotency tokens"
---

# TransactWriteItems

> **spec:trace:** specs/aws/dynamodb/dynamodb.spec.plan.md#phase-1--core-crud-poc
> **source:** AWS DynamoDB API Reference — TransactWriteItems

`TransactWriteItems` is a synchronous write operation that groups up to 100 action requests. Actions can target items in different tables (same account/Region), but no two actions can target the same item. The aggregate size cannot exceed 4 MB.

All actions complete atomically — either all succeed or all fail. Supported action types:
- **Put** — initiates a `PutItem` to write a new item (with optional condition expression)
- **Update** — initiates an `UpdateItem` to modify an existing item (with optional condition)
- **Delete** — initiates a `DeleteItem` to remove an item (with optional condition)
- **ConditionCheck** — asserts a condition without writing; fails transaction if condition not met

If any action fails, `TransactionCanceledException` is raised with `CancellationReasons` — an ordered list of reasons, one per action, describing why each was cancelled.

`ClientRequestToken` enables idempotency — multiple identical calls with the same token within 10 minutes have the same effect as one call. If parameters differ, `IdempotentParameterMismatchException` is raised.

**Required:** `TransactItems`
**Input shape:** `TransactWriteItemsInput`
**Output shape:** `TransactWriteItemsOutput` (ConsumedCapacity, ItemCollectionMetrics)
...
def transact_write_items(
    ...
    *,
    ...
    @kind:operation TransactWriteItems
