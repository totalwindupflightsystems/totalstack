---
id: "@specs/aws/dynamodb/list-tags-of-resource"
version: 1.0.0
target_lang: py
owned-by: codegen
model_pool: code-gen
...
tags: [aws, dynamodb, tags, list-tags-of-resource]
short: "ListTagsOfResource operation — list all tags for a DynamoDB resource ARN"
---
...
Lists all tags for a DynamoDB resource (table or index). Returns a paginated list of key-value tag pairs. Uses an eventually consistent read model — tag changes from TagResource/UntagResource may take a few seconds to appear.

Rate limit: 10 calls/sec/account.

**Required:** `ResourceArn`
**Optional:** `NextToken` (for pagination)
**Input shape:** `ListTagsOfResourceInput`
**Output shape:** `ListTagsOfResourceOutput`
**Output members:** `Tags`, `NextToken`
**Errors:** `ResourceNotFoundException`, `InternalServerError`

## Pagination

DynamoDB allows up to 50 tags per resource. If more than one page of tags exists, the response includes a `NextToken` for retrieval of the next page. When no more pages remain, NextToken is absent from the response.
...
def list_tags_of_resource(
    ...
    *,
    ...
    @kind:operation ListTagsOfResource
