---
id: "@specs/aws/dynamodb/batch-write-item"
version: 1.0.0
target_lang: py
owned-by: codegen
model_pool: code-gen
...
tags: [aws, dynamodb, batch-write-item]
short: "BatchWriteItem operation — put or delete up to 25 items across tables"
---

# BatchWriteItem

> **spec:trace:** specs/aws/dynamodb/dynamodb.spec.plan.md#phase-1--core-crud-poc
> **source:** AWS DynamoDB API Reference — BatchWriteItem

The `BatchWriteItem` operation puts or deletes multiple items in one or more tables. A single call can transmit up to 16 MB of data, consisting of up to 25 item put or delete operations. Individual items can be up to 400 KB once stored.

`BatchWriteItem` cannot update items — it overwrites existing items (appearing like an update). Use `UpdateItem` for true updates. Individual `PutItem` and `DeleteItem` operations within the batch are atomic, but the batch as a whole is not.

Failed operations are returned in `UnprocessedItems`. If *none* of the items can be processed due to insufficient throughput, `ProvisionedThroughputExceededException` is raised. If at least one succeeds, the response includes `UnprocessedItems` for failed ones. Retry unprocessed items with exponential backoff.

Each `PutRequest` requires an `Item` (map of attributes). Each `DeleteRequest` requires a `Key` (map of primary key attributes). All key attributes must be provided for each primary key.

**Required:** `RequestItems`
**Input shape:** `BatchWriteItemInput`
**Output shape:** `BatchWriteItemOutput` (UnprocessedItems, ItemCollectionMetrics, ConsumedCapacity)
