---
id: "@specs/aws/dynamodb/execute-transaction"
version: 1.0.0
target_lang: py
owned-by: codegen
model_pool: code-gen
...
tags: [aws, dynamodb, execute-transaction, partiql]
short: "ExecuteTransaction operation — run PartiQL statements in an atomic transaction"
---

# ExecuteTransaction

> **spec:trace:** specs/aws/dynamodb/dynamodb.spec.plan.md#phase-1--core-crud-poc
> **source:** AWS DynamoDB API Reference — ExecuteTransaction

This operation allows you to perform transactional reads or writes on data stored in DynamoDB using PartiQL. The entire transaction must consist of either read statements or write statements — you cannot mix both in one transaction. The `EXISTS` function is an exception and can be used to check conditions similarly to `ConditionCheck` in `TransactWriteItems`.

All statements execute atomically — either all succeed or all fail. A `ClientRequestToken` provides idempotency (10-minute window per AWS). If any statement fails, `TransactionCanceledException` is raised with cancellation reasons.

Each statement in `TransactStatements` contains a `Statement` (PartiQL text with `?` placeholders), optional `Parameters` list, and optional `ReturnValuesOnConditionCheckFailure`.

**Required:** `TransactStatements`
**Input shape:** `ExecuteTransactionInput`
**Output shape:** `ExecuteTransactionOutput` (Responses, ConsumedCapacity)
...
def execute_transaction(
    ...
    *,
    ...
    @kind:operation ExecuteTransaction
