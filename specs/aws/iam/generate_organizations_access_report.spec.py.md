---
id: "@specs/aws/iam/generate_organizations_access_report"
version: 1.0.0
target_lang: py
owned-by: codegen
model_pool: code-gen
...
depends_on:
  - "@specs/aws/iam/plan"
  - "@specs/aws/iam/docs/API_GenerateOrganizationsAccessReport"
---

# GenerateOrganizationsAccessReport

> **spec:trace:** specs/aws/iam/iam.spec.plan.md#operation-inventory
> **spec:id:** @specs/aws/iam/generate_organizations_access_report
> **spec:implements:** @kind:operation GenerateOrganizationsAccessReport
> **AWS Protocol:** query
> **HTTP:** POST /
> **@ref:** specs/aws/iam/docs/API_GenerateOrganizationsAccessReport.spec.md
...
|-----------|------|----------|-------------|
| EntityPath | Any  # complex shape | ✓ | The path of the Organizations entity (root, OU, or account). You can build an entity path using the known structure of y |
| OrganizationsPolicyId | Any  # complex shape |  | The identifier of the Organizations service control policy (SCP). This parameter is optional. This ID is used to generat |

## Output Shape: GenerateOrganizationsAccessReportResponse

- **JobId** (Any  # complex shape): The job identifier that you can use in the GetOrganizationsAccessReport operation.
...
```speclang
def generate_organizations_access_report(store, request: dict) -> dict:
    """Generates a report for service last accessed data for Organizations. You can generate a report for any entities (organization root, organizational unit, or account) or policies in your organization. T"""
    entity_path = request.get("EntityPath", "").strip() if isinstance(request.get("EntityPath"), str) else request.get("EntityPath")
    if not entity_path:
        raise ValidationException("EntityPath is required")

    # Auto-generated handler — operation not classified as CRUD
    return store.execute("GenerateOrganizationsAccessReport", request)
```
