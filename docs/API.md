...
> **Auto-wired providers (verified live).** The auto-wired providers
> (dynamodbstreams, s3tables, transcribe) are generated by
> `development/auto_wire_providers.py` from the assembled Speclang stores.
> Each generated provider declares its `service` name and attaches handlers
> under their exact operation names, so LocalStack's `create_dispatch_table`
> resolves them on the first request. Verified live against the emulator on
> :4566 with all three overrides set (evidence:
> `probe-evidence-ts-gap-052.txt`): every probed operation returns 2xx — or a
> typed 4xx for missing resources (e.g. `NotFoundException`) — with no
> `HTTP 500`/`_w` AttributeError. s3tables is additionally pinned to the
> totalstack variant in `totalstack/providers.py`
> (`SERVICE_PROVIDER_CONFIG.set_provider_if_not_exists("s3tables",
> "totalstack")`), because upstream core has no s3tables provider; a plain
> `make start` therefore serves the totalstack s3tables variant, while plain
> starts of dynamodbstreams and transcribe serve the upstream LocalStack core
> providers (their `:default` plux.ini entries).
...
variant with `PROVIDER_OVERRIDE_DYNAMODBSTREAMS=totalstack` (verified live —
`ListStreams` returns 200 with `{"Streams": []}`, with or without the
`TableName` filter).
...
access, etc.) are not served at all (`501`). The service is pinned to the
totalstack variant in `totalstack/providers.py`
(`SERVICE_PROVIDER_CONFIG.set_provider_if_not_exists("s3tables",
"totalstack")`) because upstream core has no s3tables provider — a plain
`make start` serves it, no override needed (verified live). Because the
service is not in the
...
| ListTagsForResource | Implemented | Returns the tag map (list→map conversion at the wire boundary); no resource existence check |
...
`PROVIDER_OVERRIDE_TRANSCRIBE=totalstack` (verified live —
`ListVocabularies` returns 200 with `{"Vocabularies": []}` and a missing job
raises a typed `NotFoundException` (HTTP 400)).
