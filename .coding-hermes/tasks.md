...
## [ ] CI — Fix integration tests + shape validator jobs: uv run recreates venv with Python 3.13
    Same root cause as the gitreins CI fix (now resolved). Both `tests` and `shape-validator`
    jobs use `uv run python -m ...` which silently recreates .venv with Python 3.13, discarding
    the `uv pip install pytest botocore` from earlier steps. Integration tests fail with
    "No module named pytest", shape validator fails with "No module named botocore".
    - [ ] Fix tests job (ci.yml:72): replace `uv run python -m pytest` with `.venv/bin/python -m pytest`
    - [ ] Fix shape-validator job (ci.yml:98): replace `uv run python development/aws-shape-validator.py` with `.venv/bin/python development/aws-shape-validator.py`
    Files: .github/workflows/ci.yml

## [ ] DOC — Add uv.lock to .gitignore
...
## [ ] DOC — Fix README badges pointing to upstream localstack/localstack
    README.md has 10 references to `localstack/localstack` in CI/Coveralls/PyPI/Docker badges.
    Replace with TotalStack equivalents or remove if not applicable.
    Files: README.md