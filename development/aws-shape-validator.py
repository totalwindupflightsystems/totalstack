...
def _call_handler(service: str, op_name: str, handler, store) -> dict:
    ...
    test_inputs = {
        ...
        'ExportCertificate': lambda store: (r := store.import_certificate(Certificate='base64-cert-data', PrivateKey='base64-key-data'), {'CertificateArn': r['CertificateArn'], 'Passphrase': b'test-pass'})[1],
        'ResendValidationEmail': lambda store: (r := store.import_certificate(Certificate='base64-cert-data', PrivateKey='base64-key-data'), {'CertificateArn': r['CertificateArn'], 'Domain': 'test.example.com', 'ValidationDomain': 'test.example.com'})[1],
        'GetAccountConfiguration': {},
        ...
        'batch.DeregisterJobDefinition': lambda store: (store.register_job_definition('batch-del-jd', 'container', container_properties={'image': 'busybox', 'command': ['echo'], 'vcpus': 1, 'memory': 128}), {'jobDefinition': store._job_definitions['batch-del-jd'].jobDefinitionArn.split('/')[-1]})[1],
        'batch.SubmitJob': {'jobName': 'batch-test-job', 'jobQueue': 'batch-test-jq', 'jobDefinition': 'batch-test-jd:1'},
        'batch.DescribeJobs': {'jobs': ['job-1']},
        'batch.ListJobs': {},
        'batch.TerminateJob': {'jobId': 'job-1', 'reason': 'test termination'},
        'batch.CancelJob': {'jobId': 'job-1', 'reason': 'test cancel'},
        ...
        'bedrock-runtime.GetAsyncInvoke': {'invocationArn': 'arn:aws:bedrock:us-east-1:123456789012:async-invoke/test-invocation'},
