...
def _call_handler(service: str, op_name: str, handler, store) -> dict:
    ...
    test_inputs = {
        ...
        'DeleteWorkspaceApiKey': lambda store: (store.create_workspace('CURRENT_ACCOUNT', 'CUSTOMER_MANAGED', ['SAML'], workspaceName='delkey-ws'), ws_id := store.workspaces()[-1].id, store.create_workspace_api_key(ws_id, 'tmp-key', 'ADMIN', 3600), {'workspaceId': ws_id, 'keyName': 'tmp-key'})[1],
        'CreateWorkspaceServiceAccount': lambda store: (store.create_workspace('CURRENT_ACCOUNT', 'CUSTOMER_MANAGED', ['SAML'], workspaceName='sa-ws'), {'workspaceId': store.workspaces()[-1].id, 'name': 'test-sa', 'grafanaRole': 'VIEWER'})[1],
        'DeleteWorkspaceServiceAccount': lambda store: (store.create_workspace('CURRENT_ACCOUNT', 'CUSTOMER_MANAGED', ['SAML'], workspaceName='delsa-ws'), ws_id := store.workspaces()[-1].id, sa := store.create_workspace_service_account(ws_id, 'tmp-sa', 'VIEWER'), {'workspaceId': ws_id, 'serviceAccountId': sa['id']})[1],
        'CreateWorkspaceServiceAccountToken': lambda store: (store.create_workspace('CURRENT_ACCOUNT', 'CUSTOMER_MANAGED', ['SAML'], workspaceName='sat-ws'), ws_id := store.workspaces()[-1].id, sa := store.create_workspace_service_account(ws_id, 'tmp-sa', 'VIEWER'), {'workspaceId': ws_id, 'serviceAccountId': sa['id'], 'name': 'test-token', 'secondsToLive': 3600})[1],
        'DeleteWorkspaceServiceAccountToken': lambda store: (store.create_workspace('CURRENT_ACCOUNT', 'CUSTOMER_MANAGED', ['SAML'], workspaceName='delsat-ws'), ws_id := store.workspaces()[-1].id, sa := store.create_workspace_service_account(ws_id, 'tmp-sa', 'VIEWER'), tok := store.create_workspace_service_account_token(ws_id, sa['id'], 'tmp-tok', 3600), {'workspaceId': ws_id, 'serviceAccountId': sa['id'], 'tokenId': tok['id']})[1],
