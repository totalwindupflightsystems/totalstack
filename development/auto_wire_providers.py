...
for _fn in sorted(os.listdir(_SVC)):
    if not _fn.endswith(".code.py") or _fn == "models.code.py":
        continue
    _stem = _fn[:-8]
    # op name must match the service model's operation name exactly:
    # str.capitalize() lowercases the rest of a token, so PascalCase stems
    # like "DescribeStream" would become "Describestream" and never dispatch
