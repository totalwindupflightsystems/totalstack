"""Auto-wired TotalStack provider for bedrock-agent."""
...
_SVC = os.path.join(_ROOT, "specs", "aws", ".speclang", "assembled", "bedrock-agent")
...
class TotalStackBedrockAgentProvider:
    """Auto-wired provider for bedrock-agent."""
...
for _op, (_method, _fn) in _HANDLERS.items():
    setattr(TotalStackBedrockAgentProvider, _method, _attach_handler(_op, _method, _fn))
