"""Auto-wired TotalStack provider for cognito-identity."""
...
_SVC = os.path.join(_ROOT, "specs", "aws", ".speclang", "assembled", "cognito-identity")
...
try:
    ...
    # no assembled specs for this service (specs/aws/.speclang/assembled/<svc>
    # is empty/missing) — keep the module importable so the provider can still
    # be selected under PROVIDER_OVERRIDE_<svc>=totalstack; handlers attach
    # once the specs are regenerated
    pass
...
class TotalStackCognitoIdentityProvider:
    """Auto-wired provider for cognito-identity."""

    def __init__(self):
        self.store = _STORE_CLS() if _STORE_CLS is not None else None
...
for _op, (_method, _fn) in _HANDLERS.items():
    setattr(TotalStackCognitoIdentityProvider, _method, _attach_handler(_op, _method, _fn))
