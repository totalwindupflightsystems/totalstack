...
_SVC = os.path.join(_ROOT, "specs", "aws", ".speclang", "assembled", "bedrock")
...
class TotalStackBedrockProvider:
    """Auto-wired provider for bedrock."""
...
for _op, (_method, _fn) in _HANDLERS.items():
    setattr(TotalStackBedrockProvider, _method, _attach_handler(_op, _method, _fn))
