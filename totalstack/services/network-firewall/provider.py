"""Auto-wired TotalStack provider for network-firewall."""
...
_SVC = os.path.join(_ROOT, "specs", "aws", ".speclang", "assembled", "network-firewall")
...
class TotalStackNetworkFirewallProvider:
    """Auto-wired provider for network-firewall."""
...
for _op, (_method, _fn) in _HANDLERS.items():
    setattr(TotalStackNetworkFirewallProvider, _method, _attach_handler(_op, _method, _fn))
